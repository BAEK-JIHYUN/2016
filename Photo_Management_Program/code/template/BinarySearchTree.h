...
public:
	...
	*/
	...
	/**
	*	@brief	n번째 노드를 반환함
	*	@pre	Tree array가 initialized 되어있어야함 
	*	@post	n번째 노드를 반환함
	*	@return	n번째 노드의 주소값
	*/
	Node<ItemType>* CallItem(int n);

	/**
	*	@brief	Tree의 node를 스크린에 출력한다.
	*	@pre	none
	*	@post	스크린에 Tree가 InOrder방법으로 출력됨.
	*/
	void PrintTree(ostream &out)const;

	/**
	*	@brief	트리에 있는 노드를 순서대로 item에 저장한다.
	*	@pre	Tree가 initialized 되어 있어야 한다.
	*	@post	트리에 있는 노드가 data가 가리키는 메모리에 저장된다.
	*	@param	item	참조할 데이터.
	*	@return 성공시 1, 실패시 0을 리턴한다.
	*/
	void WriteTree(ofstream& fout);

	void Write(Node<ItemType>* root, ofstream& fout);

private:
	Node<ItemType>* root;				// Node 타입의 root
	Node<ItemType>** arr;				// 각 노드에 직접 접근하기위해 Node를 저장하는 배열
	int index;							// index
};
...
{
	MakeEmptyTree(root,index);			// Tree 초기화 함수 호출
}
...
{
	Insert(root, item, arr, index);					 // 새 node 추가하는 함수 호출
}
...
// n번째 노드를 반환함 
template<class ItemType>
Node<ItemType>* BinarySearchTree<ItemType>::CallItem(int n) {
	return arr[n];
}
...
// Inorder 방법으로 Tree 속 노드를 파일에 입력하는 함수 
template<class ItemType>
void  BinarySearchTree<ItemType>::Write(Node<ItemType>* root, ofstream& fout) {
	if (root == NULL)
		return;
	Write(root->left, fout);
	(root->data).WriteDataToFile(fout);
	Write(root->right, fout);
}

// Inorder 방법으로 Tree 속 노드를 입력
template<class ItemType>
void  BinarySearchTree<ItemType>::WriteTree(ofstream& fout) {
	Write(root, fout);							// InOrder 방법으로 입력 
}
...
// Tree를 초기화하는 함수
template<class ItemType>
void MakeEmptyTree(Node<ItemType>*& root,int& index)
{
...
template<class ItemType>
void Insert(Node<ItemType>*& root, ItemType item, Node<ItemType>**& arr, int& index)
{
	if (root == NULL)				// root가 null일 경우 
	{
		...
		Insert(root->left, item, arr, index);	// root의 왼쪽으로 Insert 함수 다시 호출
	else if (root->data < item)					// root가 존재하고, 그 값이 새로운 item 값보다 작을 때
		Insert(root->right, item, arr, index);	// root의 오른쪽으로 Insert 함수 다시 호출
}
...
// Tree에서 node를 검색하는 함수. 주소 반환 
template <typename ItemType>
void GetAddress(Node<ItemType> *root, ItemType*& item, bool &found)
{
	if (root == NULL)						// root가 NULL인 경우 found는 false 
		found = false;
	else if (item < root->data)				// 찾고자 하는 아이템값이 root값보다 작을 때 
	...
	{										// 찾고자 하는 값과 일치할 때
		item = &(root->data);				// item에 노드 정보의 주소를 복사
		found = true;						// found값을 true로 해서 찾는 과정을 멈춤
	}
}
